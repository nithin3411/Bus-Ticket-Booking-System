...
{%>
<% 
PreparedStatement insertStatement = con.prepareStatement("INSERT INTO passdlt(uname,p_name,gender,b_id,seat_no,t_id,created_on,time_)SELECT uname, p_name,gender,b_id,seat_no,t_id,date_,time_ FROM passenger WHERE t_id = ?");
insertStatement.setString(1, t_id);
int rowsInserted = insertStatement.executeUpdate();

PreparedStatement ss=con.prepareStatement("delete from passenger where t_id=?");
ss.setString(1,t_id);
int rowsDeleted = ss.executeUpdate();


//PreparedStatement selectStmt = con.prepareStatement(" select uname,p_name,gender,b_id,seat_no,t_id,date_,time_ from passenger where t_id=?");
//selectStmt.setString(1, t_id);
//ResultSet rs1 = selectStmt.executeQuery();


//PreparedStatement insertStmt = con.prepareStatement("insert into passdlt(uname,p_name,gender,b_id,seat_no,t_id,created_on,time_) values (?,?,?,?,?,?,?,?)");
//if (rs1.next()) {
  // insertStmt.setString(1, rs1.getString("uname"));
   //insertStmt.setString(2, rs1.getString("p_name"));
   //insertStmt.setString(3, rs1.getString("gender"));
   //insertStmt.setString(4, rs1.getString("b_id"));
   //insertStmt.setString(5, rs1.getString("seat_no"));
   //insertStmt.setString(6, rs1.getString("t_id"));
   //insertStmt.setString(7, rs1.getString("created_on"));
   //insertStmt.setString(8, rs1.getString("time_"));
